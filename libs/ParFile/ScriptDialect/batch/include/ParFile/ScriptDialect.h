...
inline constexpr std::string_view PROLOGUE = "@echo off\n"
                                             "pushd \"%~dp0\"\n"
                                             "if errorlevel 1 exit /b 1\n";
inline constexpr std::string_view EPILOGUE = "popd\n";
inline constexpr std::string_view ERROR_CHECK = "if errorlevel 1 exit /b 1\n";
inline constexpr std::string_view MAKE_DIRECTORY_FORMAT = "if not exist \"{0}\" mkdir \"{0}\"\n";
inline constexpr std::string_view MOVE_FORMAT = "move /y \"{0}\" \"{1}\"\n";
