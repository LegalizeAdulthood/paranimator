...
inline constexpr std::string_view PROLOGUE = "#!/usr/bin/env bash\n"
                                             "set -e\n"
                                             "pushd \"$(dirname \"$0\")\" >/dev/null\n";
inline constexpr std::string_view EPILOGUE = "popd >/dev/null\n";
inline constexpr std::string_view ERROR_CHECK = "";
inline constexpr std::string_view MAKE_DIRECTORY_FORMAT = "mkdir -p \"{0}\"\n";
inline constexpr std::string_view MOVE_FORMAT = "mv -f \"{0}\" \"{1}\"\n";
